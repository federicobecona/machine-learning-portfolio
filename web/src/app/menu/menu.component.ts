...
export class MenuComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const menusCopy = this.MenusList.map((menu) => {
      // take this logic to utils
    ...
    while (i < list!.length) {
      ...
      auxDescription = this.DescripcionList.find(
        (desc) => desc.unit == menu?.title! && desc.task == list[i]
      );
      if (lang == 'en') {
      ...
      button.addEventListener('click', function () {
        ...
        const sanitizedMenuName = encodeURIComponent(
          menuName!.toLowerCase().replace(/\s+/g, '-')
        );
        const sanitizedBlogName = encodeURIComponent(
          blogName.toLowerCase().replace(/\s+/g, '-')
        );
        let newURL = router.createUrlTree([
          sanitizedMenuName,
          sanitizedBlogName,
        ]);
